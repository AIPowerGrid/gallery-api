...
func Generate(prompt string, seed int64, jobid string) error {
...
func apiExample(b []byte) (map[string]interface{}, error) {
	var result map[string]interface{}
	url := fmt.Sprintf("%s/prompt", apiUrl)
...
func GetImage(filename string) (string, error) {
	url := fmt.Sprintf("%s/view?filename=%s", apiUrl, filename)
	log.Info(url)
